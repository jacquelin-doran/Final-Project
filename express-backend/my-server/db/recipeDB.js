...
class RecipeDB {
    ...
    static insertRecipe(recipe) {
        return new Promise((resolve, reject) => {
            this.db.run(`INSERT INTO Recipes (id, title, instructions, directions) VALUES ("${recipe.id}", "${recipe.title}", "${recipe.instructions}", "${recipe.directions}")`, function(err, data) {
                resolve(recipe)
            })
        })
    }
